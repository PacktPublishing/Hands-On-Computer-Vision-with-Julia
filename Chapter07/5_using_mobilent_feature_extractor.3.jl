...
features = zeros((299, 299, 3, length(images)))

for idx = 1:250:length(images)
    
    println(idx)

    row_count = min(length(images) + 1 - idx, 250)
    mx_data_features = mx.zeros((299, 299, 3, row_count));

    for idx_ = 1:row_count
        features[:, :, :, idx:idx + row_count - 1] = Float16.(channelview(RGB.(imresize(load(images[idx + idx_ - 1][1]), (299, 299)))));
    end
...
for i = 1:30
    ...
    mx.fit(nnet, optimizer, train_data_provider, eval_data = validation_data_provider, n_epoch = 1, callbacks = [mx.speedometer()]);
